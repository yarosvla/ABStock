...
///
/// Scoped (как у <see cref="IActiveAssetContext"/>) переходы по ссылкам внутри
/// приложения переживает: enhanced navigation не перезагружает документ и не
/// рвёт контур. Но перезагрузку страницы и вход по прямому адресу — нет, а
/// симуляция при этом продолжает идти. В рельсе оказалось бы «Новостей 0»
/// посреди сессии, в которой новости были.
/// </summary>
