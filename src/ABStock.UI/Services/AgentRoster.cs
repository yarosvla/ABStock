...
/// </summary>
/// <param name="TradingNowCount">
/// Сколько агентов торгуют сейчас. Признак — открытая позиция: агент в рынке
/// ровно тогда, когда у него на руках есть бумага.
///
/// До этого здесь считались «торговали за сессию» — агенты с деньгами,
/// отличными от стартовых. Это метрика за период, и стояла она между
/// «капиталом под управлением» (на сейчас) и «сделками за сессию» (за
/// период), где читалась третьим периодом (раздел 10). Артборд 01 просит
/// «торгуют сейчас», и это заодно полезнее: видно, что новостные агенты без
/// новостей простаивают.
///
/// «Действует на этом шаге» сюда не годится: решение меняется каждый тик, и
/// число прыгало бы между 0 и 2 из 9, ничего не сообщая.
/// </param>
...
public static class AgentRoster
{
        ...
        IReadOnlyList<AgentDecision> decisions)
    {
        ...
        return new AgentRosterView(
            ...
            agents.Count(agent => agent.Position != 0m));
    }
