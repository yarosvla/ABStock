...
public sealed class DebugSimulationRunner : ISimulationRunner, ISimulationDebugControl
{
    ...
    public Task StartAsync(SimulationConfig config, CancellationToken ct = default)
    {
        ...
        lock (_sync)
        {
            ...
            EnsureUniqueAgentNames(_agents);
    ...
    private SimulationTickResult RunTickLocked()
    {
        ...
        var decisions = _agents
            .Select(agent => agent.Decide(snapshot, news))
            .ToList();
    ...
    private static IReadOnlyList<AgentSnapshot> GetAgentSnapshots(
        IReadOnlyList<ITradeAgent> agents,
        decimal lastPrice) =>
        agents.Select(agent => new AgentSnapshot(
            agent.State.AgentName,
            agent.State.AgentType,
            agent.State.Cash,
            agent.State.Position,
